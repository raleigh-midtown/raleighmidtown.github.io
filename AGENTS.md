...
Lazy-loaded in the build-time fetch script (`scripts/fetch-osm.ts`): `osmtogeojson`. Eagerly loaded: `three`, `three-mesh-bvh`, `proj4js`. The browser never calls Overpass — it loads the bundled `/data/midtown.geojson` produced by that script (cache-first via `localStorage`); `npm run dev`/`build` auto-run the script through the `predev`/`prebuild` hooks.
...
│   └── data/midtown.geojson           # generated by scripts/fetch-osm.ts (gitignored); served at /data/midtown.geojson
├── src/
...
│   ├── osm/
│   │   ├── fetch.ts                     # browser loader: cache-first over /data/midtown.geojson (cache key v2)
│   │   ├── overpassQuery.ts             # Overpass query builders (shared w/ scripts/fetch-osm.ts; DOM-free)
...
├── scripts/
│   └── fetch-osm.ts                    # Node build-time Overpass → public/data/midtown.geojson (run via predev/prebuild)
├── docs/
