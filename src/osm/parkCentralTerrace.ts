 ...
 * rises from ground at the west trim (local X=0) to the deck height over
 * `RAMP_LEN`, then stays flat at `TERRACE_H` to the building's NE corner
 * (local X=L). The east end (X=L) is a sheer vertical face; the west end is the
 * ramped descent. Extruding along Z turns each profile edge into a lateral
 * face: the ramp edge becomes the sloped walkable ramp, the flat edge becomes
 * the deck top, and the east edge becomes the sheer wall — all without a
 * `rotateX(-π/2)` (the shape is already Y-up; that flip is only for footprint
 * extrudes in `extrude.ts`).
 *
 * Placement follows AGENTS.md "Hand-placed scene props belong inside a
 * THREE.Group": one group positioned at the west trim ON the wall and rotated
 * `JROT` once, so local +X runs east along the wall and local -Z is the wall's
 * outward normal (toward the road). Children use local coordinates.
 *
 ...
 * ~15 m north of the wall, ~parallel to it. The real northern limits are the
 * adjacent road "Park At North Hills Street" (~10 m north of the wall) and the
 * park polygon (~15 m north). The deck extends only ~4.7 m north of the wall
 * (road-side front), leaving ~5.3 m of road clearance and ~10 m of park
 * clearance, so R2's intent (don't cross the park / stay south of the road)
 * holds while giving the full-depth deck + 12 m ramp the plan asked for. The
 * literal z≥263 bound is unsatisfiable with R3+R5 (see plan risk note) and is
 * superseded by these real limits per the user's 2026-06-30 decision.
 *
 ...
 *   West trim at s_trim=72, so the 12 m ramp spans s∈[72,84] (west of Jubala,
 *   Jubala sits on the flat top s∈[86,100] with ≥2 m west margin) and the flat
 *   deck runs s∈[84,128.1] to the NE corner. Group origin = B + s_trim·u =
 *   (271.79, 263.04); L = 128.1 − 72 = 56.1 m.
 *
...
// ── Terrace geometry ─────────────────────────────────────────────────────────
const TRIM_S = 72;          // along-wall distance (m) from west end to the trim (group origin)
...
const TERRACE_LEN = WALL_LEN - TRIM_S;       // 56.1 — flat + ramp span to the NE corner

// Group origin: the trim point ON the wall.
const GROUP_X = WALL_WEST_X + TRIM_S * WALL_UX; // 271.79
const GROUP_Z = WALL_WEST_Z + TRIM_S * WALL_UZ; // 263.04
...
export function buildParkCentralTerrace(scene: THREE.Scene): THREE.Mesh[] {
  ...
  // West trim (X=0) is ground level; the surface ramps up to TERRACE_H over
  // RAMP_LEN, then stays flat to the NE corner (X=L). The east edge (X=L) is a
  // sheer drop to ground (R4). Extruding along +Z sweeps each profile edge into
  // a lateral face: ramp edge → sloped walkable ramp, flat edge → deck top,
  // east edge → sheer wall.
